...
exports.create = function(req, res) {
	...
	} if (!newItem.madv || !newItem.tendv || !newItem.dongia || !newItem.mota) {		
		return res.status(400).send('One or more required fields are missing')
	} else {		
		Item.create(newItem, function(err, id) {
			console.log('err: ', err);
			//if (err === Object) res.status(500).send('Item already exist with name ' + err.item_name);
			
			if (err || id <= 0) return res.status(500).send('Error occured during saving item');
